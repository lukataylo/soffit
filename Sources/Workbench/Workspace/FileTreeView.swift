    ...
    private var quickSections: some View {
        VStack(alignment: .leading, spacing: 1) {
            ...
            sectionRow(icon: "clock.fill", label: "Recent", tint: Color(red: 0.38, green: 0.56, blue: 0.92)) {
                openFolder(workspace.root) // stub; honors Recent via sort-by-modified in the folder panel
            }
    ...
    private func entryRow(_ entry: FSEntry, depth: Int) -> AnyView {
        if entry.isDirectory {
            ...
            return AnyView(
                ...
                .contextMenu {
                    Button("Open Preview") { openFile(entry.url, mode: .preview) }
                    Button("Open in Editor") { openFile(entry.url, mode: .split) }
                    Divider()

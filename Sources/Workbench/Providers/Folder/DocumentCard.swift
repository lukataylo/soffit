...
struct DocumentCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            contentBody
                .padding(14)
                .frame(maxWidth: .infinity, minHeight: 160, maxHeight: 220, alignment: .topLeading)
                .clipped()
        }
        .background(
        ...
        .shadow(color: .black.opacity(isHovered ? 0.10 : 0.06), radius: isHovered ? 12 : 6, x: 0, y: isHovered ? 5 : 2)
        ...
        .gesture(
            TapGesture(count: 2)
                .onEnded { onDouble() }
                .exclusively(before:
                    TapGesture(count: 1).onEnded { onSingle() }
                )
        )
